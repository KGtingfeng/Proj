...
namespace FairyGUI
{
    ...
    public class GList : GComponent
    {
        ...
        /// <summary>
        /// 针对单行 横向滑动的对象 暂时只适用于角色成长 其他地方没有测过
        /// </summary>
        /// <param name="index">Index.</param>
        public void ScrollToViewMiddle(int index)
        {
            bool ani = false; 
            bool setFirst = false;
            if (_virtual)
            {
                ...
                Rect rect;
                ItemInfo ii = _virtualItems[index];
                if (_layout == ListLayoutType.SingleRow || _layout == ListLayoutType.FlowVertical)
                {
                    float pos = 0;
                    for (int i = _curLineItemCount - 1; i < index; i += _curLineItemCount)
                        pos += _virtualItems[i].size.x + _columnGap;

                    pos -= ii.size.x * 0.5f - _columnGap*2.5f;
                    rect = new Rect(pos, 0, ii.size.x, _itemSize.y);
